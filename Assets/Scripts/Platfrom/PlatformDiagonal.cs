...
public class PlatformDiagonal : Platform
{
    protected override void NextPoint()
    {
        ...
        else
        {
            if (!up)
            {
                prevPoint = new Vector3(pointOne.position.x, pointOne.position.y, transform.position.z);
                nextPoint = new Vector3(pointTwo.position.x, pointTwo.position.y, transform.position.z);
            }
            else
            {
                prevPoint = new Vector3(pointTwo.position.x, pointTwo.position.y, transform.position.z);
                nextPoint = new Vector3(pointOne.position.x, pointOne.position.y, transform.position.z);
            }
        }
